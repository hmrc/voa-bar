...
class ReportUploadService @Inject() (
  ...
  voEbarsConnector: VOEbarsConnector,
  ...
  def upload(baLogin: LoginDetails, xmlUrl: String, uploadReference: String)(using headerCarrier: HeaderCarrier): Future[String] =
    val processingResult =
      for
        ...
        _           <- EitherT.fromEither[Future](xmlValidator.validateAsDomAgainstSchema(submissions)) // XML Schema validation
  ...
  def upload(baLogin: LoginDetails, baReports: BAreports, uploadReference: String)(using headerCarrier: HeaderCarrier): Future[String] =
    val processingResult =
      for
  ...
  private def handleUploadResult(result: Future[Either[BarError, String]], login: LoginDetails, uploadReference: String)(using headerCarrier: HeaderCarrier)
    : Future[String] =
    result
      .recover {
  ...
  private def downloadAndFixXml(url: String)(using hc: HeaderCarrier): EitherT[Future, BarError, BAreports] =
    ...
    def parseXml(rawXml: Array[Byte]): Either[BarError, BAreports] =
      for
        _         <- xmlValidator.validateInputXmlForXEE(CorrectionInputStream(ByteArrayInputStream(rawXml)))
        baReports <- unmarshal(rawXml)
      yield baReports

    def unmarshal(rawXml: Array[Byte]): Either[BarError, BAreports] = Try {
      val source = StreamSource(CorrectionInputStream(ByteArrayInputStream(rawXml)))
  ...
  private def handleError(submissionId: String, barError: BarError, login: LoginDetails): Future[?] =
    ...
    def handleValidationErrors(errors: List[models.Error]) =
  ...
  private def eBarsUpload(baReports: BAreports, login: LoginDetails, submissionId: String)(using hc: HeaderCarrier): Future[Either[BarError, Boolean]] =
    def internalUpload(jsonString: String): Future[Either[BarError, Boolean]] =
      ...
      voEbarsConnector.sendBAReport(req).map(_ => Right(true)).recover {
