...
class XmlParser:
  ...
  val documentBuilderFactory: DocumentBuilderFactory = DocumentBuilderFactory.newInstance("org.apache.xerces.jaxp.DocumentBuilderFactoryImpl", null)
  ...
  def parse(xml: Array[Byte]): Either[BarError, Document] =
    Try {
      val docBuilder = documentBuilderFactory.newDocumentBuilder()
      docBuilder.parse(ByteArrayInputStream(xml))
    } match
      case Success(value) => Right(value)
      case Failure(x)     => Left(BarXmlError(x.getMessage))

  private def addChild(node: Node, newNode: NodeSeq): Node =
